...
public class HttpParser {
    ...
    private static final int SP = 0x20; //20
        ...
        while ((_byte = reader.read()) >= 0){
            if (_byte == CR){
                ...
                if (_byte == LF){
                    ...
                    } catch (BadHttpVersionExeption e) {
                        throw new HttpParsingException(HttpStatusCode.CLIENT_ERROR_400_BAD_REQUEST);
                    }
            ...
            if (_byte == SP){
                if (!methodParsed){
                    LOGGER.debug("Request Line METHOD to Parse: {}",processingDataBuffer.toString() );
                    request.setMethod(processingDataBuffer.toString());
                    methodParsed = true;
                }else if(!requestTargetParsed){
                    LOGGER.debug("Request Line REQUEST TARGET to Parse: {}",processingDataBuffer.toString() );
